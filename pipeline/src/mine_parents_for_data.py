...
def build_series_to_lat_lng(*, images, geocoded, id_to_series, series_geocoded):
    ...
    for parent, lst in lat_lng_candidates.items():
        ...
        if any(
            p
            for p, _ in info.get("part_of_links", [])
            if p == URBAN_DESIGN_PHOTOGRAPHS_ID
        ):
            continue
...
def main(*, images, geocoded, id_to_series, series_geocoded, output_handle):
    series_to_lat_lng = build_series_to_lat_lng(
        images=images,
        geocoded=geocoded,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="pipeline/dist/series.ndjson",
    )
    parser.add_argument(
        "--geocoded_results",
        type=str,
    ...
    parser.add_argument(
        ...
        default="pipeline/dist/series-geocoded.json",
    )
    parser.add_argument(
        "--images",
        type=str,
        help="ndjson formatted file containing all images",
        default="pipeline/dist/images.ndjson",
    )
    parser.add_argument(
        "--output",
        type=str,
        help="json file to write out the mined values keyed by image id",
        default="pipeline/dist/parent_mined_data.json",
    )
    ...
    print(
        f"read {len(id_to_series)} series,"
