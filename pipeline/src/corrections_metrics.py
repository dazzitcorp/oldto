...
def check_against_truth_data(geocodes, truth):
    ...
    for row in truth:
        ...
        if geocode_result:
            ...
            is_same, error_reason = diff_geocode(
                (float(row["lng"]), float(row["lat"])), (geocode_lng, geocode_lat)
            )
            if is_same:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default="pipeline/dist/corrections.geocode_results.json",
    )
    parser.add_argument(
        "--truth_data",
        type=str,
        help="CSV file (without a header line) of imageId,lat,lng",
        default="pipeline/dist/corrections.csv",
    )
    parser.add_argument(
        "--incorrect_file",
        type=str,
        help="file to write a csv of image_id,error_reason",
        default="pipeline/dist/incorrect.csv",
    )
