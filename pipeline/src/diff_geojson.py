...
def diff_geojson(
    ...
    for k in new_ids.intersection(old_ids):
        ...
        if (
            get_lat(old[k]) is not None
            and get_lat(new[k]) is not None
            and get_lng(old[k]) is not None
            and get_lng(new[k]) is not None
            and distance > 25
        ):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Analyze the differences between two sets of geocodes."
    )
