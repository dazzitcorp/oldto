...
python pipeline/geocode.py --ids "$(cat data/unstructured_feedback.tsv | awk -F'\t' '{ print $2}' | paste -s -d ',')" --output /tmp/geocodes.json
python pipeline/generate_geojson.py --geocode_results /tmp/geocodes.json --output /tmp/images.geojson
python pipeline/diff_geojson.py --sample_set "$(cat data/unstructured_feedback.tsv | awk -F'\t' '{ print $2}' | paste -s -d '|')" "${original_geojson}" /tmp/images.geojson --num_samples 10
